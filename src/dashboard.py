"""Dashboard Flask — lecture seule sur SQLite.

Routes :
    GET /           → redirige vers /log
    GET /log        → historique des décisions (50 dernières)
    GET /inbox      → emails score=4 (action requise) en attente
    GET /stats      → distribution des scores et règles
    POST /inbox/<id>/snooze  → snooze un item reply_queue (date form param)
    POST /inbox/<id>/done    → marque un item reply_queue comme traité

Usage :
    python src/dashboard.py [--port 5000] [--db path/to/postroom.db]
"""
import argparse
import sqlite3
import sys
...
def log():
    ...
    try:
        rows = _rows(conn, """
            SELECT d.decided_at, e.account, e.sender, e.subject,
